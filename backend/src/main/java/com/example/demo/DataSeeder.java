...
public class DataSeeder implements CommandLineRunner {
    ...
    private List<Plan> createPlans() {
        List<Plan> plans = List.of(
            new Plan("Basic", new BigDecimal("29.99"), 5, 500, 500),
    ...
    private List<Customer> createCustomers(List<Plan> plans) {
        String[][] customerData = {
            {"Sarah", "Johnson", "sarah.johnson@email.com", "+1-555-0101"},
    ...
    private List<Device> createDevices(List<Customer> customers) {
        String[] models = {"iPhone 15 Pro", "iPhone 15", "Samsung Galaxy S24", "Samsung Galaxy A54", 
                          "Google Pixel 8", "OnePlus 12", "Motorola Edge", "iPhone 14"};
            ...
            for (Customer customer : activeCustomers) {
                for (UsageRecord.Type type : types) {
                    if (random.nextDouble() > 0.3) {
                        ...
                        records.add(new UsageRecord(customer, type, quantity, cost.setScale(2, java.math.RoundingMode.HALF_UP), recordDate));
                    }
                }
            }
        }
    ...
    private void createSupportTickets(List<Customer> customers) {
        String[][] ticketData = {
            {"Billing inquiry", "I have a question about my last bill"},
            ...
            {"Voicemail setup", "Help setting up voicemail"},
            {"Payment issue", "Payment failed, need assistance"},
